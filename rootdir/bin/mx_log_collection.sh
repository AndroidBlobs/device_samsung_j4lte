...
xml_dir=$base_dir/$fw_var$fw_suffix/mx140/debug/hardware/moredump
log_strings=$base_dir/$fw_var$fw_suffix/mx140/debug/common/log-strings.bin

take_moredump()
{
memdump_file=1
if [ -f /data/vendor/conn/.memdump.info ]; then
	memdump_file=`cat /data/vendor/conn/.memdump.info`
	echo "/data/vendor/conn/.memdump.info : ${memdump_file}"  >> ${status_file} 2>&1
fi

if [[ ${memdump_file} != "0" ]]; then
	start=`date +%s`
...
# copy .sbl file
mv .tmp-${trigger}.sbl ${logdir}/${trigger}.sbl 2>&1
echo "copied .tmp-${trigger}.sbl to ${logdir}/${trigger}.sbl" >> ${status_file} 2>&1
